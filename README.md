...
## Дополнение: Реализация автоматического мониторинга статуса соединения и построение графика

### 9. Проверка результата failover

Был создан Python-скрипт app.py, который подключается	каждые 2 секунды к БД (standby на порту 5434). При успешном соединении записывает в failover_log.csv статус “success”, а при ошибке подключения (при отказе standby) фиксирует ошибку и пишет “FAILOVER DETECTED”.

Формат CSV:
timestamp,status,message
2025-04-22T17:32:53.522621,success,Connected
2025-04-22T17:41:54.014560,error,FAILOVER DETECTED: Connection refused
...

### 10. Визуализация логов
Разработан скрипт status_graph.py, который читает failover_log.csv и строит график изменения состояния соединения во времени.
1 — соединение успешно
0 — ошибка соединения / failover
График позволяет наглядно увидеть момент отказа и восстановления соединения.

![График изменения статуса соединения](kursovaya/status_timeline.png)
